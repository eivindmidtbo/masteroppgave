...
def _fun_wrapper_corr_bucketing(args):
    ...
    city, res, layers, true_sim_matrix, measure, size, bucketing_method = args
...
# Varying layers
def compute_grid_corr_varying_layers_bucketing(
    city: str,
    layers: list[int],
    true_sim_matrix,
    ...
    for layer in layers: # Varying parameter
        corrs = pool.map(
            ...
            [
                (city, resolution, layer, true_sim_matrix, measure, size, bucketing_method)
...
def plot_grid_corr_varying_layers_bucketing(
     city: str,
    layers: list[int],
    true_sim_matrix,
    resolution: float = 1.6,
    ...
    results = compute_grid_corr_varying_layers_bucketing(
        city=city, layers=layers, true_sim_matrix=true_sim_matrix, resolution = resolution, size=size, bucketing_method=bucketing_method, measure=measure, parallel_jobs=parallel_jobs
    )
...
# Varying resolution
def compute_grid_corr_varying_resolution_bucketing(
    city: str,
    layers: int,
    true_sim_matrix,
    ...
    for res in np.arange(*resolution): # Varying parameter
        corrs = pool.map(
            _fun_wrapper_corr_bucketing,
            [
                (city, res, layers, true_sim_matrix, measure, size, bucketing_method)
                for _ in range(parallel_jobs)
            ],
        )
        corr = np.average(np.array(corrs))
...
def plot_grid_corr_varying_resolution_bucketing(
    city: str,
    layers: int,
    true_sim_matrix,
    resolution: list[float],
    ...
    results = compute_grid_corr_varying_resolution_bucketing(
        city=city, layers=layers, true_sim_matrix=true_sim_matrix, resolution = resolution, size=size, bucketing_method=bucketing_method, measure=measure, parallel_jobs=parallel_jobs
    )
...
def compute_grid_corr_varying_resolution_and_layers_bucketing(
    city: str,
    ...
    for layer in layers: # Varying parameter
        ...
        for res in np.arange(*resolution):
            ...
            corrs = pool.map(
                _fun_wrapper_corr_bucketing,
                [
                    (city, res, layer, true_sim_matrix, measure, size, bucketing_method)
                    for _ in range(parallel_jobs)
                ],
            )
...
def plot_grid_corr_varying_resolution_and_layers_bucketing(
    city: str,
    layers: list[int], #Varying parameter
    true_sim_matrix,
    resolution: list[float], #Varying parameter
    ...
    results = compute_grid_corr_varying_resolution_and_layers_bucketing(
        city=city, layers=layers, true_sim_matrix=true_sim_matrix, resolution = resolution, size=size, bucketing_method=bucketing_method, measure=measure, parallel_jobs=parallel_jobs
    )
