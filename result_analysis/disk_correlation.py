...
import numpy as np
import pandas as pd
from computation.similarity import generate_disk_hash_similarity
...
def _fun_wrapper_corr(args):
    ...
    city, diameter, layers, disks, true_sim_matrix, measure, size = args
    
    hashed_similarities = generate_disk_hash_similarity(
        city=city,
        diameter=diameter,
        disks=disks,
        layers=layers,
...
# Varying layers
def compute_disk_corr_varying_layers(
    city: str,
    layers: list[int],
    true_sim_matrix,
    diameter: float = 1.6,
    ...
    for layer in layers: # Varying parameter
        corrs = pool.map(
            _fun_wrapper_corr,
            [
                (city, diameter, layer, disks, true_sim_matrix, measure, size)
...
def plot_disk_corr_varying_layers(
    city: str,
    layers: list[int],
    true_sim_matrix,
    diameter: float = 1.6,
    ...
    results = compute_disk_corr_varying_layers(
        city=city, layers=layers, true_sim_matrix=true_sim_matrix, diameter= diameter, disks=disks, size=size, measure=measure, parallel_jobs=parallel_jobs
    )
...
def compute_disk_corr_varying_disks(
    city: str,
    layers: int,
    true_sim_matrix,
    ...
    for num_disks in disks: # Varying parameter
        corrs = pool.map(
            _fun_wrapper_corr,
            [
                (city, diameter, layers, num_disks, true_sim_matrix, measure, size)
                for _ in range(parallel_jobs)
            ],
        )
        corr = np.average(np.array(corrs))
...
def plot_disk_corr_varying_disks(
    city: str,
    layers: int,
    true_sim_matrix,
    disks: list[int], #Varying parameter
    ...
    results = compute_disk_corr_varying_disks(
        city=city, layers=layers, true_sim_matrix=true_sim_matrix, diameter= diameter, disks=disks, size=size, measure=measure, parallel_jobs=parallel_jobs
    )
...
#Varying diameter
def compute_disk_corr_varying_diameter(
    city: str,
    layers: int,
    true_sim_matrix,
    disks: int, 
    ...
    for dia in np.arange(*diameter): # Varying parameter
        ...
        corrs = pool.map(
            _fun_wrapper_corr,
            [
                (city, dia, layers, disks, true_sim_matrix, measure, size)
                for _ in range(parallel_jobs)
            ],
        )
        corr = np.average(np.array(corrs))
...
def plot_disk_corr_varying_diameter(
    city: str,
    layers: int,
    true_sim_matrix,
    disks: int, 
    ...
    results = compute_disk_corr_varying_diameter(
        city=city, layers=layers, true_sim_matrix=true_sim_matrix, diameter=diameter, disks=disks, size=size, measure=measure, parallel_jobs=parallel_jobs
    )
...
def compute_disk_corr_varying_size(
    city: str,
    layers: int,
    true_sim_matrix,
    disks: int, 
    ...
    for size in sizes: # Varying parameter
        corrs = pool.map(
            _fun_wrapper_corr,
            [
                (city, diameter, layers, disks, true_sim_matrix, measure, size)
                for _ in range(parallel_jobs)
            ],
        )
        corr = np.average(np.array(corrs))
...
def plot_disk_corr_varying_size(
    city: str,
    layers: int,
    true_sim_matrix,
    disks: int, 
    ...
    results = compute_disk_corr_varying_size(
        city=city, layers=layers, true_sim_matrix=true_sim_matrix, diameter=diameter, disks=disks, sizes=sizes, measure=measure, parallel_jobs=parallel_jobs
    )
...
def plot_disk_corr_varying_diameter_and_layers(
    ...
    results = compute_disk_corr_varying_diameter_and_layers(
        city=city, layers=layers, true_sim_matrix=true_sim_matrix, diameter= diameter, disks=disks, size=size, measure=measure, parallel_jobs=parallel_jobs
    )
...
def compute_disk_corr_varying_num_of_disks_and_layers(
    city: str,
    layers: list[int], #Varying parameter
    true_sim_matrix,
    diameter: float,
    ...
    for layer in layers: # Varying parameter
        ...
        for num_disks in disks:
            ...
            corrs = pool.map(
                _fun_wrapper_corr,
                [
                    (city, diameter, layer, num_disks, true_sim_matrix, measure, size)
                    for _ in range(parallel_jobs)
                ],
            )
...
def plot_disk_corr_varying_num_of_disks_and_layers(
    city: str,
    layers: list[int], #Varying parameter
    true_sim_matrix,
    diameter: float,
    ...
    results = compute_disk_corr_varying_num_of_disks_and_layers(
        city=city, layers=layers, true_sim_matrix=true_sim_matrix, diameter=diameter, disks=disks, size=size, measure=measure, parallel_jobs=parallel_jobs
    )
...
def compute_disk_corr_varying_num_of_disks_and_diameter(
    city: str,
    layers: int, 
    true_sim_matrix,
    diameter: list[float], #Varying parameter
    ...
    for num_disks in disks: # Varying parameter
        ...
        for dia in np.arange(*diameter):
            ...
            corrs = pool.map(
                _fun_wrapper_corr,
                [
                    (city, dia, layers, num_disks, true_sim_matrix, measure, size)
                    for _ in range(parallel_jobs)
                ],
            )
...
def plot_disk_corr_varying_num_of_disks_and_diameter(
    city: str,
    layers: list[int], #Varying parameter
    true_sim_matrix,
    diameter: int,
    ...
    results = compute_disk_corr_varying_num_of_disks_and_diameter(
        city=city, layers=layers, true_sim_matrix=true_sim_matrix, diameter=diameter, disks=disks, size=size, measure=measure, parallel_jobs=parallel_jobs
    )
