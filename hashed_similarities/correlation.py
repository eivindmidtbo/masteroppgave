...
def compute_correlation_similarity(city: str, scheme: str, runs: int):
    ...
    porto_dtw = _mirrorDiagonal(pd.read_csv(os.path.abspath("../benchmarks/similarities/porto-dtw-testset.csv"), index_col=0)).flatten()
    porto_fre = _mirrorDiagonal(pd.read_csv(os.path.abspath("../benchmarks/similarities/porto-frechet-testset.csv"), index_col=0)).flatten()
...
#    }
    ...
    print("DTW:", min(correlation_dtw), max(correlation_dtw), np.average(correlation_dtw), np.std(correlation_dtw))
    print("FRE:", min(correlation_fre), max(correlation_fre), np.average(correlation_fre), np.std(correlation_fre))
